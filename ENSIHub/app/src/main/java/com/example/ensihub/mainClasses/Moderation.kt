        ...
        db.collection("posts").whereEqualTo("status", PostStatus.PENDING.name).get()
            .addOnSuccessListener { result ->
                for(document in result) {
                    ...
                    val post = Post(data["id"] as Long, data["text"] as String, data["timestamp"] as Long, data["author"] as String, data["likesCount"] as Long, data["imageUrl"] as String?, data["videoUrl"] as String?, data["status"] as PostStatus)
